...
void initializePorts(ADC * a0, ADC *  a1, ADC * a2){
	...
	IOWR_ALTERA_AVALON_PIO_DATA(CSE2, 1);
	IOWR_ALTERA_AVALON_PIO_DATA(CSE3, 1);
	//ADC0
	a0->readDataR = 0;
	a0->vin = 0;
	//ADC1
...
void readadc(ADC * a, int * isHigh, int pin) {
	...
	for (i=0; i<8; i++){
		...
		if(pin ==1) {
			...
			if (IORD_ALTERA_AVALON_PIO_DATA(MISO2) == 1){ // printf("1");
			data |= 1;}
			else { // printf("0");
			data &= ~1;}
		} else {
			if (IORD_ALTERA_AVALON_PIO_DATA(MISO3) == 1){ // printf("1");
			data |= 1;}
			else { // printf("0");
			data &= ~1;}
		}
