...
void GUI_DeskDraw(VIP_FRAME_READER *pReader, RECT* rc_button){
	...
	OPTIONS[0] = "Battery Capacity (Ah)";
	OPTIONS[1] = "Battery Level (%)";
	OPTIONS[2] = "Panel Voltage (V)";
	OPTIONS[3] = "Time Left (min)";
...
//    RectInflate(&rcTouch, -nDotSize-2, -nDotSize-2);
    ...
    while(1){
    	...
    	if(UPDATE_CURRENT_FLAG) {
    		...
    		average = average / 3000.0;
    		average_current = average;
    		current_charge -= average / (60 * 60);
    		if(current_charge > 9.8) {
    			current_charge = 9.72375;
    		}
    		floating_bat_level = (int)(current_charge * 100) / 9.72375;
    		if(floating_bat_level > 100) {
    			floating_bat_level = 100.0;
    		}
...
//    	printf("iin is : %d\n", a1.iin);
    	...
    	if(UPDATE_GRAPH_FLAG) {
    	    ...
    	    vid_print_string_alpha(OFFSET_HORIZ_GRAPH_INFO,
    	        	        	    			...
    	        	        	    			TEXT_COLOR_OPTION,
    	        	        	    			BG_COLOR,
    	        	        	    			tahomabold_20, pReader, "000000");
        	sprintf(max_char, "%4.2f V", ((float)(max_y) / (float) (1280)) * 5 * 28);
    	    vid_print_string_alpha(OFFSET_HORIZ_GRAPH_INFO,
    	        	    			OFFSET_VERT_GRAPH_INFO + 35,
    	        	    			TEXT_COLOR_OPTION,
    	        	    			BG_COLOR,
